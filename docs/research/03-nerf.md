...
Neural Radiance Fields (NeRF) — метод представления сцены в виде непрерывной функции, отображающей пространственные координаты и направление наблюдения в цвет и плотность. NeRF стал одним из ключевых направлений neural rendering и показал высокое качество восстановления сложных сцен по набору 2D-изображений.
...
Адаптация NeRF для портретов с контролируемой мимикой.
...
- **Разнообразие:** множество углов, освещения и выражений лица
...
### Ограничения по мимике

- Сложность в анимации выражений лица
- Требуется специальный датасет для управления выражениями лица
- Интерполяция между выражениями не всегда корректна
...
NeRF обеспечивает высокое качество рендера новых ракурсов, но требует значительных вычислительных ресурсов и обычно множества изображений. Для практического применения 3D-реконструкции лица по 1–3 фотографиям со смартфона классический NeRF не подходит как основной baseline, хотя neural rendering, 3D Gaussian Splatting и диффузионные prior-модели остаются перспективными для R&D.
