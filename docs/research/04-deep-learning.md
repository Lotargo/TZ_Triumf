...
Deep learning подходы к 3D-реконструкции лица позволяют восстанавливать трёхмерную геометрию из одного или нескольких изображений с помощью свёрточных нейросетей. Ключевые модели этого направления — DECA, EMOCA и MICA — обеспечивают высокую детализацию при minimal входных данных.

## DECA (Detailed Expression Capture and Animation)

### Архитектура

DECA используетEncoder-Decoder архитектуру с двумя ветвями:
...
- Chamfer Distance: 0.82 мм (BIWI), 0.91 мм (MEAD)
- Normal Consistency: 0.91 (BIWI), 0.88 (MEAD)
- Фото-реалистичность: SSIM 0.89
...
1. **Улучшенное восстановление expressions**
   - Более точная модель expression (50→75 параметров)
   - Лучшая generalization на сильные expressions
...
def emotion_loss(predicted, target):
    # Специальный loss для expressions
...
- Дополнительные: детекция landmarks, классификация expression

## MICA (Monocular Image 3D avatar)
...
### Качество на benchmarks

| Benchmark | DECA | EMOCA | MICA |
|-----------|------|-------|------|
| BIWI (Chamfer) | 0.82 мм | 0.78 мм | 0.71 мм |
| MEAD (Chamfer) | 0.91 мм | 0.85 мм | 0.88 мм |
| AFLW2000 (NME) | 2.8% | 2.6% | 2.9% |
| Florence (SSIM) | 0.87 | 0.89 | 0.85 |
...
- **BIWI:** Студийное освещение, нейтральные expressions
...
- Мелкие details (поры, морщины) poorly восстанавливаются
...
- Устойчивость к изменению expression
