...
```
Тренировочные данные:     Реальные данные:
- Студийное освещение  →  - Естественное освещение
- Равномерное          →  - Тени, блики
- Нейтральная температура → - Тёплые/холодные оттенки
```
...
```
Тренировочные данные:     Реальные данные:
- Frontal (±15°)      →  - Profile, extreme angles
- Стабильная поза      →  - Динамическая поза
```
...
```
Тренировочные данные:     Реальные данные:
- Нейтральный         →  - Эмоциональные
- Ограниченный набор   →  - Спонтанные expressions
- Синхронизированный   →  - Размытие motion blur
```

**Проявления:**
- Искажение expression parameters
...
- Низкий шум          →  - Высокий шум
- Без артефактов      →  - JPEG compression
```
...
```
Тренировочные данные:     Реальные данные:
- Без аксессуаров    →  - Очки, шляпы, серьги
- Открытое лицо       →  - Частичная occlusion
```
...
Domain gap остаётся significant проблемой для deep learning моделей 3D-реконструкции лица. Основные стратегии борьбы:
